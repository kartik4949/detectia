...
def test_sanity_efficientnet(backbone):
    config = Config()
    _default_head_shapes = [[13, 13], [26, 26], [52, 52]]
    config.backbone = backbone
    model = builder.ModelBuilder(config)
    detections = model(tf.random.uniform(shape=(1, 416, 416, 3)))
    out_head_shapes = [list(out.shape[1:3]) for out in detections]
